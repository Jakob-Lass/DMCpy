...
import os, copy
import json, os, time
from DMCpyZEBRA import DataFile, _tools, Viewer3D, RLUAxes, TasUBlibDEG
from DMCpyZEBRA.FileStructure import shallowRead, HDFCountsBG, HDFTranslation
import warnings
import DMCpyZEBRA
...
class DataSet(object):
    ...
    def Viewer3D(self,dqx,dqy,dqz,rlu=True,axis=2, raw=False,  log=False, grid = True, outputFunction=print, 
        ...
        Kwargs:
            ...
            -5 grid (bool): Plot a grid on the figure (default True)
    ...
    def binData3D(self,dqx,dqy,dqz,rlu=True,raw=False,smart=False,steps=10):
        ...
        for df in self:
            ...
            for idx in _tools.arange(0,len(df),steps):
                ...
                print(f'{df.fileName} from {idx[0]} to {idx[-1]}')
                stepsTaken+=steps
    ...
    def plotCut1D(self,P1,P2,rlu=True,stepSize=0.01,width=0.05,widthZ=0.05,raw=False,optimize=True,ax=None,steps=None,**kwargs):
        ...
        """
        if self[0].instrGeometry == 'bi':
            warnings.warn('1D cut optimization is not supported in bissecting geometry. Using unoptimized algorithm.')
            optimize = False
